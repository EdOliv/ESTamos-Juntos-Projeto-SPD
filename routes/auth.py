...
def reset_password():
  ...
  try:
    ...
    login_data = request_data["login"]
    login = login_schema.load(login_data)  # type: "Login"
    ...
    duplicate_user = Login.find_by_username(login.username)
    if duplicate_user:
      message = f"Login {login.username} already registered"
      return jsonify(message=message), 400

    if not is_valid_city_state_country(
            user.city_id,
            user.state_id,
            user.country_id):
      return jsonify({"message": "Invalid location"}), 400

    new_account = Account(created_by=user)
    login.account = new_account

    organization.accounts.append(new_account)
    print(user.type)
    res = User.create(user)
    if not res:
      return jsonify({"message": "An error occurred while signing up"}), 400

    user = user_org_schema.dump(user)
    login = login_schema.dump(login)

    identity = {'login': login["username"],
                'name': user["name"], 'id': user["id"]}
    access_token = create_access_token(identity=identity)
    response = {
        'access_token': access_token,
